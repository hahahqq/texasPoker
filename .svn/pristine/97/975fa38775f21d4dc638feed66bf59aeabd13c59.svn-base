    ...
    <div class="f-botton-fssock" style="height: 100%;">
            ...
            <el-row :gutter="24" style="height: 100%;">
                    ...
                    <el-form label-width="80px" class="fastc_top">
                        ...
                        <el-form-item label="业绩员工">
                            <el-input
                                ...
                                :disabled="true"
                                class="Choiceinput"
                                v-model="ruleFormsock.AllselecteSaleEmpId"
                                @click.native="selecteSaleEmpId"
                            ></el-input>
                        </el-form-item>
                ...
                <el-col :span="8">
                    <div style="margin-top:10px;margin-left:12px;">
                        <div style="font-size:15px;color:#78797A;">充值赠送内容:</div>
                        <div>
                            <el-row style="color:#78797A;margin-top:5px;">1、赠送金额<span>{{giveMoney}}</span>元;</el-row>
                            <el-row style="color:#78797A;margin-top:5px;">2、赠送<span>{{integral}}</span>积分;</el-row>
                            <el-row style="color:#78797A;margin-top:5px;">3、赠送<span></span>次卡</el-row>
                            <el-row v-for="item in goodList" :key="item.ID" class="goodList">
                                <el-col :span="8" class="goodList-Img">
                                    <img :src="item.showgoodsimg"/>
                                </el-col>
                                <el-col :span="8" style="margin-left:8px;">
                                    <div>{{item.NAME}}</div>
                                    <div style="color:#78797A;font-size:8px;">{{item.Day}}</div>
                                </el-col>
                                <el-col :span="8" style="text-align:center">x{{item.QTY}}</el-col>
                            </el-row>
                        </div>
        ...
        <el-dialog title="选择业绩员工" :visible.sync="dialogFormVisible">
            <div class="employeetitle">
                充值金额：
                <i class="com_color">{{payMoney}}</i>元，按金额的
                <i class="com_color">{{ruleFormsock.getfastcExtract}}</i>提成，员工可的
                <i class="com_color">{{ruleFormsock.ExtractMoney}}</i>元提成
            </div>
                    ...
                    <el-form :inline="true" :model="ruleForm[0]" class="fastc_top">
                        <el-col :span="8">
                            <el-form-item label="员工" class="half" width="30%">
                                <el-select
                                    v-model="ruleForm[0].SaleEmpIdfirst"
                                    placeholder
                                    class="full-width"
                                    @change="SaleEmpIdfirst"
                                >
                                    <el-option
                ...
                <el-col :span="24">
                    <el-form :inline="true" :model="ruleForm[1]" class="fastc_top">
                        <el-col :span="8">
                            <el-form-item label="员工" class="half" width="30%">
                                <el-select
                                    v-model="ruleForm[1].SaleEmpIdsecond"
                                    placeholder
                                    class="full-width"
                                    @change="SaleEmpIdsecond"
                                >
                                    <el-option
                                        v-for="(item,i) in employeeList"
                                        :key="i"
                ...
                <el-col :span="24">
                    <el-form :inline="true" :model="ruleForm[2]" class="fastc_top">
                        <el-col :span="8">
                            <el-form-item label="员工" class="half" width="30%">
                                <el-select
                                    v-model="ruleForm[2].SaleEmpIdthird"
                                    placeholder
                                    class="full-width"
                                    @change="SaleEmpIdthird"
                                >
                                    <el-option
                                        v-for="(item,i) in employeeList"
                                        :key="i"
...
export default {
    ...
    watch: {
        storagevaluerroyaltyState(data) {
            if (data.success) {
                this.ruleFormsock.getfastcExtract =
                    data.data.EmpAddRate * 100 + "%";
                this.ruleFormsock.ExtractMoney = (
                    this.payMoney * data.data.EmpAddRate
                ).toFixed(2);
            }
        },
        storagevaluerrechargeState(data) {
            if (data.success) {
                ...
                if (this.$store.state.recharge.isPrintsatus) {
                    getDayindate("601020305", data.data.BillId, 4, qresurl);
                }
            } else {
    ...
    methods: {
        ...
        inputpayMoney() {
            //支付金额
            console.log("12435235")
            this.giveMoney = 0
            this.integral = 0;
            this.goodList=[];

            console.log("ev.marketingRechargedetailed")
            console.log(this.marketingRechargedetailed)
            let carList = this.marketingRechargedetailed.data.CountList;
            let moneyList = this.marketingRechargedetailed.data.GiftList;
            let ruleId;
            for(var i in moneyList){
                if(this.payMoney >= moneyList[i].MONEY){
                    this.giveMoney = moneyList[i].ADDMONEY;
                    ruleId = moneyList[i].ID;
                    this.integral = moneyList[i].INTEGRAL;
                }
            }
            this.getgoodsIMGURL = GOODS_IMGURL;
            for(var j in carList){
                if(ruleId==carList[j].DETAILID){
                    let obj={
                        QTY:carList[j].QTY,
                        NAME:carList[j].GOODSNAME,
                        Day:carList[j].DAY == 40000? '不限期' : carList[j].DAY,
                        ID:carList[j].GOODSID,
                        showgoodsimg:this.getgoodsIMGURL + carList[j].GOODSID + ".png"
                    }
        ...
        Rechargeok() {
            ...
            if (this.payMoney == "") {
                ...
                if (getparametersetup(2)) {
                    if (this.SaleEmpMoneyArray.length == 0) {
                        this.$message("请选择业绩员工");
                        return;
                    }
                }
            }
        ...
        CashRecharge(data) {
            ...
            this.$store
                .dispatch("getstoragevaluerrechargeState", AllsendData)
                .then(() => {});
        },
        selecteSaleEmpId() {
            if (this.payMoney == "") {
                ...
                this.$store
                    .dispatch("getstoragevaluerroyaltyState", {})
                    .then(() => {});
        ...
        SaleEmpIdthird(vId) {
            if (
                ...
                this.ruleForm[0].Extract =
                    Math.ceil(this.ruleFormsock.ExtractMoney * 33) / 100;
                this.ruleForm[1].Extract =
                    Math.ceil(this.ruleFormsock.ExtractMoney * 33) / 100;
                this.ruleForm[2].Extract =
                    Math.ceil(this.ruleFormsock.ExtractMoney * 33) / 100;
        ...
        getAllSaleEmpId() {
            ...
            if (this.ruleForm[0].SaleEmpIdfirst != "") {
                ...
                let NamePrice =
                    this.ruleForm[0].Name + this.ruleForm[0].Extract + "元";
            ...
            if (this.ruleForm[1].SaleEmpIdsecond != "") {
                ...
                let NamePrice =
                    this.ruleForm[1].Name + this.ruleForm[1].Extract + "元";
                showEmpIdsock.push(NamePrice);
            }
            if (this.ruleForm[2].SaleEmpIdthird != "") {
                ...
                let NamePrice =
                    this.ruleForm[2].Name + this.ruleForm[2].Extract + "元";
                showEmpIdsock.push(NamePrice);
            }
