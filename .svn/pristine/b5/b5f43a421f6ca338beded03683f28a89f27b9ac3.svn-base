...
var ListARR = {
  coupon:{
    ...
    ItemCode:'601020824',
    CodeStop:'210021025',
    paying:{ "TotalNumber": 0, "PageNumber": 0, "PageSize": 20, "PN": 0 },List:[]
  },
  member:{InterfaceCode:'',paying:{
    "TotalNumber": 0,
    "PageNumber": 0,
    "PageSize": 20,
    "PN": 0,
  },List:[]},
  sms:{InterfaceCode:'',paying:{
    "TotalNumber": 0,
    "PageNumber": 0,
    "PageSize": 20,
    "PN": 0,
...
// actions
const actions = {
  getBespeakDateList({commit}, data){  // 预约日期列表
  ...
  getRecommendList({commit}, data){  //推荐人报表
    let homeInfo = getHomeData()
    let sendData = {
  ...
  getIncomeItem({commit}, data){  //推荐人收益详情
    let homeInfo = getHomeData()
    let sendData = {
  ...
  getIncomeVipInfo({commit}, data){  // 推荐人详情
    let homeInfo = getHomeData()
    let sendData ={
  ...
  saveRGiftsSet({commit}, data){  //分享有礼保存
    let homeInfo = getHomeData()
    let sendData = {
  ...
  getApplicationList({commit}, data){   //提现申请列表
    let homeInfo = getHomeData()
    let sendData = {
  ...
  setApplicationStep({commit}, data){  //确认申请单(601020242)
    let homeInfo = getHomeData()
    let sendData = {
  ...
  saveSpecials({commit}, data){  // 保存限时特价
    let homeInfo = getHomeData()
    let sendData = {
  ...
  getSpecialsList({commit}, data){  // 限时特价列表
    let homeInfo = getHomeData();
    let sendData = {
  ...
  getSpecialsItem({commit}, data){  // 限时特价详情
    let homeInfo = getHomeData()
    let sendData = {
  ...
  delSpecials({commit}, data){  // 限时特价删除
    let homeInfo = getHomeData()
    let sendData = {
  ...
  //发布充值有礼
  getMarketingRechargeAdd({commit,state}, data) {
    console.log(data.data.ShopList)
    let homeInfo = getHomeData();
    let sendData = {
  ...
  //活动充值修改
  getMarketingRechargeEit({commit,state}, data) { //
    console.log(data.data.ShopList)
    let homeInfo = getHomeData();
    let sendData = {
  ...
  dealPromotionAction({commit},{type,data}) { //发布优惠活动, 新增和编辑
    let homeInfo = getHomeData();
    let sendData={
      'InterfaceCode':'',
      'CompanyId':homeInfo.shop.COMPANYID,
    ...
    switch(type){
      case 'add': sendData.InterfaceCode="210021001";   break;
      case 'edit': sendData.InterfaceCode='210021005';sendData.BillId = data.BillId; break;
      // case 'del': break;
    }
    commonSend.commonSend('get', data => {
  ...
  dealGoodsAction({commit,state},{type,data}) { //发布优惠商品, 新增和编辑
    let homeInfo = getHomeData();
    let sendData={
      'InterfaceCode':'',
      'CompanyId':homeInfo.shop.COMPANYID,
    ...
    switch(type){
      case 'add': sendData.InterfaceCode="210021011";   break;
      case 'edit': sendData.InterfaceCode='210021005';sendData.BillId = data.BillId; break;
      // case 'del': break;
    }
    commonSend.commonSend('get', data => {
  ...
  dealcouponAction({commit},{type,data}) { //发行优惠券, 新增和编辑
    let homeInfo = getHomeData();
    let sendData={
      'InterfaceCode':'210021021',
      'CompanyId':homeInfo.shop.COMPANYID,
      ImgName:data.ImgName,
    ...
    // switch(type){
    //   case 'add': sendData.InterfaceCode="210021021";   break;
    //   // case 'edit': sendData.InterfaceCode='210021005';sendData.BillId = data.BillId; break;
    //   // case 'del': break;
    // }
    commonSend.commonSend('get', data => {
  ...
  clearMarketingData({state},type){
    switch(type){
      case 1: break;
      case 2: state.marketingItem = {}; break;
      default:
      state.marketingList=[]
  ...
  addMCoyponAction({commit},{type,data}) { //发行定向优惠券
    let homeInfo = getHomeData();
    let sendData={
      'InterfaceCode':'210021021_01',
      'CompanyId':homeInfo.shop.COMPANYID,
      ImgName:data.ImgName,
      ShopList: data.ShopList,
      Money:data.Money,
  ...
  setSMSAction2({commit},data) { // 短信群发
    let homeInfo = getHomeData();
    let sendData={
      'InterfaceCode':'2100126',
      MobileList:data.MobileList,
  ...
  getRegisterGiftsData({commit}, data){  // 注册有礼列表
    let homeInfo = getHomeData();
    let sendData = {
  ...
  DealRegisterGifts({commit}, {type, data}){  // 注册有礼修改、发布
    let homeInfo = getHomeData();
    let sendData = {
  ...
  getRegisterGiftsItem({commit}, data){  // 注册有礼详情
    let homeInfo = getHomeData()
    let sendData = {
  ...
  delRegisterGiffs({commit}, data){  // 注册有礼删除
    let homeInfo = getHomeData()
    let sendData = {
  ...
  setVipCardBg({commit}, data){ // 会员卡设置
    let homeInfo = getHomeData()
    let sendData = {
  ...
  getVipCardBg({commit}, data){ // 获取会员卡信息
    let homeInfo = getHomeData()
    let sendData = {
  ...
  getIntegralResetList({commit}, data){  // 积分清零列表
  ...
  delIntegralReset({commit}, data){  // 积分清零删除
    let userInfo = getUserInfo()
    let homeInfo = getHomeData();
    let sendData = {
  ...
  saveIntegralReset({commit}, data){   // 积分清零保存
    let userInfo = getUserInfo()
    let homeInfo = getHomeData();
    let sendData = {
  ...
  detailIntegralReset({commit}, data){  // 积分清零详情
    let userInfo = getUserInfo()
    let homeInfo = getHomeData();
    let sendData = {
...
const mutations = {
  ...
  [GET_REGISTERGIFTS_DATA](state, {data}){  // 注册有礼列表
    if(data.success){
  ...
  [MARKETING_LIST] (state, { data }) {
    ...
    state.marketingListState = Object.assign({},data,{'paying':pageData,'List':state.marketingList});
  },
