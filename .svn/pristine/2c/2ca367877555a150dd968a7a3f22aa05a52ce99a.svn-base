...
let selected={}
var ListARR = [
  { InterfaceCode:'6010312',title:'余额积分统计',paying:{ "TotalNumber": 0, "PageNumber": 0, "PageSize": 20, "PN": 0,  },List:[]  },
]
...
const actions = {
  getintegralBalanceList ({commit},{obj,data}) { //余额积分统计
    ...
    let pn = parseInt(state.integralBalanceListState.paying.PN) +1;
  ...
  getnumberBalanceList({commit,state},data) { // 调整余次统计
    let userInfo= getUserInfo();
    let pn = parseInt(state.numberBalanceListState.paying.PN) +1;
    let sendData = {
