    ...
    <div class="YJemployee_tplone">
      <div class="shoptitle">
        ...
        <div class="itmeright">
          ...
          <div>
            <strong class="com_color" style="padding-right: 20px;">￥{{shopitemList.Price|money}}</strong>
        ...
        <el-form :inline="false" ref="ruleForm" label-width="80px">
          <el-form-item label="员工" prop="Name" class="half" :class="{'serverhalf':shopitemList.itemObj.GOODSMODE==1}">
            <el-select v-model="ruleForm[0].SaleEmpIdfirst" placeholder="" @change="SaleEmpIdfirst" clearable>
              <el-option v-for="(item,i) in employeeList" :key="i" :label="item.NAME+' ('+item.CODE+')'" :value="item.ID" :disabled="disabledFun(item.ID)"></el-option>
            </el-select>
          </el-form-item>
          ...
          <el-form-item label="提成" prop="Name" class="half" :class="{'serverhalf':shopitemList.itemObj.GOODSMODE==1}">
            <el-input v-model="ruleForm[0].Extract" :disabled="!isPurViewFun(601050209)"></el-input>
          </el-form-item>
          ...
          <el-form-item label="提成" prop="Name" class="half" :class="{'serverhalf':shopitemList.itemObj.GOODSMODE==1}">
            <el-input v-model="ruleForm[1].Extract" :disabled="!isPurViewFun(601050209)"></el-input>
          </el-form-item>
          ...
          <el-form-item label="提成" prop="Name" class="half" :class="{'serverhalf':shopitemList.itemObj.GOODSMODE==1}">
            <el-input v-model="ruleForm[2].Extract" :disabled="!isPurViewFun(601050209)"></el-input>
          </el-form-item>
        </el-form>
      </div>
    </div>
...
export default {
  ...
  methods: {
     disabledFun: function(id) {
           if(this.ruleForm[0].SaleEmpIdfirst==id) return true;
           if(this.ruleForm[1].SaleEmpIdsecond==id) return true;
           if(this.ruleForm[2].SaleEmpIdthird==id) return true;
            return false;
        },
    SaleEmpIdfirst(vId) {
