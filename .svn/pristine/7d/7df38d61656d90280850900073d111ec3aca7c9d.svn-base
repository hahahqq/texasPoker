...
let selected={}
var ListARR = [
  { InterfaceCode:'6010303_03_01',title:'支付方式明细',paying:{ "TotalNumber": 0, "PageNumber": 0, "PageSize": 20, "PN": 0,  },List:[]  },
]
...
const state = {
  checkoutReportState: {},
   checkoutReportData: {},
   checkoutReportList:[],
   checkoutReportListState:{paying:{
    "TotalNumber": 0,
    "PageNumber": 0,
    "PageSize": 20,
    "PN": 0,
...
const actions = {
  ...
  getcheckoutReportPayData ({commit},data) { // 支付方式列表
    let userInfo= getUserInfo();
    let homeInfo = getHomeData();
    let sendData = {
      'InterfaceCode': '6010303_03',
      'CompanyId': userInfo.CompanyID,
      ShopId:data.ShopId?data.ShopId:'',
      BeginDate:data.BeginDate?data.BeginDate :1,
      EndDate:data.EndDate?data.EndDate : 9999999999999,
  ...
  getcheckoutReportList ({commit,state},{obj,data}) { // 支付方式明细
    let userInfo= getUserInfo();
    let homeInfo = getHomeData();
...
const mutations = {
  ...
  [GET_CHACKOUT_LIST](state, { data }) {
    ...
    state.checkoutReportListState = Object.assign({},data,{'paying':pageData,'List':state.checkoutReportList});
    selected = {}
  },
