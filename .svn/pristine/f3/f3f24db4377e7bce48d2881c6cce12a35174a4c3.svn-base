    ...
    <div class='history_top'>
      <el-row>
        ...
        <el-col :span="4" style='text-align:right'>
          <el-input size='small' v-model='searchText' placeholder="输入单号、店铺" @input='searchList(searchText)' clearable></el-input>
        </el-col>
        <el-col :span="4" style='text-align:right'>
          <el-date-picker
            size="small"
      ...
      <el-table 
        ...
        <el-table-column label="操作" fixed='right' width="100">
          <template slot-scope="scope">
            ...
            <el-button size='small' v-if='scope.row.ISCHECK == true && scope.row.ISCANCEL == false && scope.row.ISFLAG == true' type='text' @click='cancelBill(scope.row.BILLID, "zf")'>作废</el-button>
            <el-button size='small' v-if='scope.row.ISCHECK == true && scope.row.ISCANCEL == false && scope.row.ISFLAG == false' type='text' @click='viewCurInfo(scope.row, scope.$index)'>调入</el-button>
            <el-button size='small' v-if='scope.row.ISCHECK == false && scope.row.ISCANCEL == false && scope.row.ISFLAG == false' type='text' @click='cancelBill(scope.row.BILLID,"sc")'>删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog :title='`详情 (${title})`' :visible.sync="isShowDialog" width="80%" append-to-body>
      ...
      <el-table
        ...
        <el-table-column width="100" label="金额" align="center" >
          <template slot-scope='scope'>
            <span>{{isPurViewFun(210040601) ? (scope.row.PRICE * scope.row.QTY) : '****'}}</span>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods:{
    ...
    console.log("9999999999999",this.dataList)
    let GoodsList=[]
        for(let i in this.dataList){
          let obj={
            Qty:this.dataList[i].QTY,
            Id:this.dataList[i].ID
          }
          GoodsList.push(obj)
        }
      if(isPrintStatu =='打印'){
        getDayindate('601021242', BillId, 16, '');
      }else if( isPrintStatu == '确认调入'){
        
        this.$store.dispatch('saveAllocationBill', {BillId: BillId,GoodsList:JSON.stringify(GoodsList)})
      } else{
    ...
    handleDraft(editStatu, BillId){  //弹窗列表是否作废状态
      if(editStatu == '作废'){
        ...
        this.$store.dispatch('cgdAllocationDataList', sendData).then(()=>{
            this.isShowDialog = false
