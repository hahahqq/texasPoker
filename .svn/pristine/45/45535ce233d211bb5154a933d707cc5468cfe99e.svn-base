              ...
              <template slot-scope="scope">
                <span
                  v-if="
                    ...
                    scope.row.ISCANCEL == false &&
                    scope.row.ISFLAG == true
                  "
                >
                  已调拨
                </span>
            ...
            <el-table-column label="操作" fixed="right" width="100">
              <template slot-scope="scope">
                ...
                <el-button
                  ...
                  "
                  type="text"
                  @click="viewCurInfo(scope.row, scope.$index)"
                >
                  调入
                </el-button>
                <el-button
  ...
  methods: {
    ...
    handlePrint(isPrintStatu, BillId, item) {
      ...
      if (isPrintStatu == "打印") {
        ...
        this.$store.dispatch("saveAllocationBill", {
          BillId: BillId,
    ...
    curRadioStatus() {
      ...
      if (this.radioStatus == 0) {
        (IsCheck = -1), (IsCancel = -1), (IsFlag = -1);
      } //全部
      else if (this.radioStatus == 1) {
        (IsCheck = 1), (IsCancel = 0), (IsFlag = 1);
      } // 已调拨
      else if (this.radioStatus == 2) {
        (IsCheck = 1), (IsCancel = 0), (IsFlag = 0);
      } // 待调入
      else if (this.radioStatus == 3) {
        (IsCheck = 0), (IsCancel = 0), (IsFlag = -1);
      } // 草稿单
      else if (this.radioStatus == 4) {
        (IsCheck = 1), (IsCancel = 1), (IsFlag = -1);
      } // 已作废
