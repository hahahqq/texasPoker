...
let selected={}
var ListARR = [
  { InterfaceCode:'60102100604',title:'员工排行-好评详情',paying:{ "TotalNumber": 0, "PageNumber": 0, "PageSize": 20, "PN": 0,  },List:[]  },
 ]
...
// actions
const actions = {
  getemployeeSortData ({commit},data) {  // 员工排行-好评
  ...
  getemployeeSortList ({commit,state},{obj,data}) { // 员工排行-好评详情
    let userInfo= getUserInfo();
  ...
  getEmployeeAchievement ({commit},data) {  // 员工排行-业绩
    let userInfo= getUserInfo();
    let sendData = {
  ...
  getEmployeeBespeak ({commit},data) {  // 员工排行-预约
    let userInfo= getUserInfo();
    let sendData = {
...
const mutations = {
  ...
  [GET_EMPLOYEE_SORT_LIST](state, { data }) {
    ...
    state.employeeSortListState = Object.assign({},data,{'paying':pageData,'List':state.employeeSortList});
    selected = {}
  },
