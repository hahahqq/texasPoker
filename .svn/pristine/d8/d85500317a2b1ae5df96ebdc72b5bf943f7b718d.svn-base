...
import { 
  GET_MEMBERREPORT_DATA,
  GET_MEMBERREPORT_LIST,
} from '@/store/mutation-types'
let selected={}
var ListARR = [
  { InterfaceCode:'6010306_01',title:'会员分析明细',paying:{ "TotalNumber": 0, "PageNumber": 0, "PageSize": 20, "PN": 0,  },List:[]  },
]

// initial state
const state = {
  memberReportState: {},
  memberReportData: {paying:{
    "TotalNumber": 0,
    "PageNumber": 0,
    "PageSize": 20,
    "PN": 0,
...
const actions = {
  getmemberReportData ({commit},data) { 
    ...
    let pn = parseInt(state.memberReportData.paying.PN) +1;
    let sendData = {
      'InterfaceCode': '21002072201',
  ...
  getmemberReportList ({commit,state},{obj,data}) { // 明细
    let userInfo= getUserInfo();
    let homeInfo = getHomeData();
...
const mutations = {
  ...
  [GET_MEMBERREPORT_LIST](state, { data }) {
    ...
    state.memberReportListState = Object.assign({},data,{'paying':pageData,'List':state.memberReportList});
    selected = {}
  },
}
