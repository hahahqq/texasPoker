<template>
    <section class="mallOrder-item bg-white p-bottom-sm" v-loading="loading">
                ...
                <el-col :span="7" class="text-center border-right">
                    <div style="height:160px;">
                        ...
                        <div class="paddingLR-sm paddingTB-xs">
                            ...
                            <div v-if="dataInfo.STATUS==2||dataInfo.STATUS==3||dataInfo.STATUS==4">
                                <el-button
                                    size="small"
                                    type="danger"
                                    plain
                    ...
                    <div style="height:160px;">
                        ...
                        <div v-if="dataInfo.STATUS>2">
                            <div class="paddingLR-sm paddingTB-xs">
                                ...
                                <span v-if="dataInfo.LOGISTICSID">{{dataInfo.LOGISTICSCOMPANYNAME}}</span>
                                <span v-else>无</span>
                            </div>
                            <div class="paddingLR-sm paddingTB-xs">
                    ...
                    <thead class="box-shadow">
                        <tr class="bg-f8 text-4e">
                            <th
                                ...
                                :key="idx"
                                :class="(idx==1||idx==2)?'text-center':(idx==tableHead.length-1?'text-right':'text-left')"
                            >{{item.label}}</th>
                        </tr>
                    </thead>
                        ...
                        <tr v-for="(item, i) in goodsList" :key="i">
                            ...
                            >
                                ...
                                <span
                                    ...
                                    class="pull-right"
                                    :style="item.STATUS>0?(item.STATUS==3?'color:green':'color:red'):'color:#aaa'"
                                >{{refundStateList[item.STATUS]}}</span>
                            </td>
                        </tr>
                    </tbody>
                    <tfoot>
                        ...
                        <tr v-if="dataInfo.STATUS==1">
                            ...
                            <td width="80px" class="text-right">-&yen;{{orderChangedMoney|money}}</td>
                        </tr>
                        <tr>
                            <td :colspan="tableHead.length-2" class></td>
                            <td width="120px">优惠券：</td>
                            <td width="80px" class="text-right">
                                <span v-if="dataInfo.COUPONMONEY">-&yen;{{dataInfo.COUPONMONEY}}</span>
                        ...
                        <tr>
                            ...
                            <td width="80px" class="text-right">&yen;{{dataInfo.FREIGHTMONEY}}</td>
                        </tr>
                        <tr style="color:red;">
                            <td :colspan="tableHead.length-2" class></td>
                            <td width="120px">应付总额：</td>
                            <td width="80px" class="text-right font-14">&yen;{{dataInfo.PAYMONEY}}</td>
                        </tr>
                    </tfoot>
                </table>
            </div>
        ...
        <!-- 修改金额 -->
        <el-dialog title="修改订单金额" :visible.sync="dialogVisible1" append-to-body width="450px">
            <div>
                <el-form ref="form" :model="formMoney" :rules="rules" label-width="80px">
                    ...
                    <el-form-item>
                        <el-button size="small" type="primary" @click="handleButton('1-edit')">确认</el-button>
        ...
        <!-- 修改地址 -->
        <el-dialog title="修改收货地址" :visible.sync="dialogVisible2" append-to-body width="600px">
            <address-page
        ...
        <!-- 退款 -->
        <el-dialog title="申请退款" :visible.sync="showRefund" append-to-body width="500px">
            <order-refund-page
                :pageData="dialogData"
        ...
        <!-- 发货 -->
        <el-dialog title="产品发货" :visible.sync="showDeliverGoods" append-to-body width="600px">
            <deliverGoods-page @closeModal="showDeliverGoods=false"></deliverGoods-page>
        </el-dialog>
        <!-- 打印小票 -->
        <el-dialog title="打印小票" :visible.sync="showPrint" append-to-body width="400px">
            <div style="line-height: 40px;">
                <div class="text-center">商家名称</div>
                <div class="text-center relative">
                    <div class="translateY-center full-width" style="height:1px;background:red"></div>
                    <div class="paddingLR-sm relative bg-white inline-block" style="z-index:1">订单信息</div>
                </div>
                <div>
                    <span>订单编号</span>
                ...
                <div class="text-center relative">
                    <div class="translateY-center full-width" style="height:1px;background:red"></div>
                    <div class="paddingLR-sm relative bg-white inline-block" style="z-index:1">消费明细</div>
                </div>
                <div>
                    <div class="p-left-md relative">
                        <span calss="inline-block absolute" style="position: absolute;left:0">1.</span>
                        <div>
                            <span>商品名称</span>
                            <span class="pull-right">&yen;10</span>
                        </div>
...
export default {
    ...
    methods: {
        ...
        printFun() {
            // this.showPrint = true;
            this.$store
                .dispatch("getQRcodeMall", { shopId: this.dataItem.Obj.SHOPID })
                .then(() => {
                    this.loading = true;
                });
        },
        ...
        countDownTime() {
            let str = "";
            let edate = this.getCustomDay(
                this.mallData.AUTOORDER,
                this.dataInfo.BILLDATE,
                true
            );
            ...
            this.stateInfo.msg =
                "需付款：￥" + this.dataInfo.PAYMONEY + "，剩余：" + str;
        },
        defaultData() {
