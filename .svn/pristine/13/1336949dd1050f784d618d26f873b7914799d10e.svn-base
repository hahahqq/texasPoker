          ...
          <div class="content-table-center">
              ...
              >
                ...
                <el-table-column label="商品" fixed='left'>
                  <template slot-scope="scope">
                    <span v-if='scope.row.isEdit &&  scope.row.GOODSID != ""'>{{scope.row.GOODSNAME}}</span>
                    <span v-if='!scope.row.isEdit && scope.row.GOODSID != ""'>{{scope.row.GOODSNAME}}</span>
                    <el-select
                      size="small"
                ...
                <el-table-column label="折扣" align="center" width="140" >
                  <template slot-scope='scope'>
                    ...
                    <el-input-number v-if='scope.row.isEdit' size='small' style="width:120px" :min='0' controls-position="right" v-model.trim="scope.row.DISCOUNT" placeholder="请输入" @change="changePr(scope.row)"></el-input-number>
                  </template>
                </el-table-column>
                <el-table-column label="单价" align="center" width="150">
                  <template slot-scope="scope">
                    <span v-if='!scope.row.isEdit'>{{isPurViewFun(210040601) ? scope.row.PRICE : '****'}}</span>
                    <el-input-number v-if='scope.row.isEdit' size='small' style="width:130px" controls-position="right" v-model.trim="scope.row.PRICE" placeholder="请输入" @change="changeDP(scope.row)"></el-input-number>
                  </template>
                </el-table-column>
                <el-table-column label="数量" align="center" width="140">
                  <template slot-scope='scope'>
                    <span v-if='!scope.row.isEdit'>{{scope.row.QTY}}</span>
                    <el-input-number v-if='scope.row.isEdit' size='small' style='width:120px' controls-position="right" :min="1" v-model.trim="scope.row.QTY" placeholder="请输入" @input="totalfun"></el-input-number>
                  </template>
                </el-table-column>
                <el-table-column label="金额" align="center" width="120" >
                  <template slot-scope='scope'>
                    {{isPurViewFun(210040601) ? (scope.row.PRICE * scope.row.QTY).toFixed(2) : '****'}}
                  </template>
                </el-table-column>
                <el-table-column label="操作" width="80" fixed='right'>
                  <template slot-scope='scope'>
                    <el-button v-if='scope.row.GOODSID != ""' type="text" size="small" @click="handleDel(scope.$index, scope.row)">删除</el-button>
                  </template>
                </el-table-column>
              </el-table>
            </div>
            ...
            <el-row :gutter="10" class="marginTB-sm">
              <el-col :xs="24" :sm="12" :md="6" class="row-flex flex-items-center">
                ...
                <el-input size='small' type='number' style="width:100%" v-model.number="pageData.BREAKSMONEY" @input="totalfun" @keyup.enter.native="totalfun" placeholder="￥ 0.00"></el-input>
              </el-col>

              <el-col :xs="24" :sm="12" :md="6" class="row-flex flex-items-center">
                <span class="leftLabel">其它费用</span>
                <el-input size="small" type='number' v-model.number="pageData.OTHERMONEY" @input="totalfun" @keyup.enter.native="totalfun" placeholder="￥ 0.00"></el-input>
              </el-col>

              <el-col :xs="24" :sm="12" :md="12" class="row-flex flex-items-center">
                <span class="inline-block text-center"> <i style="color:#868686">总合计</i>  <i class="font-600 text-danger m-left-md font-18"> {{isPurViewFun(210040601) ? '&yen; ' +totalPrice : '****'}} </i></span>
              </el-col>
            </el-row>
            <el-row>
              <el-col :xs="24" :sm="12" :md="6" class="row-flex flex-items-center">
                <span class="leftLabel">结算账户</span>
                <el-select size="small" style="width: 100%" v-model="pageData.PAYTYPEID" placeholder="选择账户">
                  <el-option
            ...
            <div class='marginTB-sm'>
              ...
              <span class='text-right' style='float:right'>
                ...
                <el-button type="info" @click="handleSubmit(0)" :disabled="theGoodsList[0].GOODSNAME == ''" :loading="CGloading">草稿</el-button>
                <el-button type="primary" @click="handleSubmit(1)" :disabled="theGoodsList[0].GOODSNAME == ''" :loading="loading">保存</el-button>
              </span>
            </div>
          </div>
        </div>
...
export default {
  ...
  watch: {
    ...
    goodsListNew(data){
      ...
      let param = this.theGoodsList, info = data[0]
      let info1 = [{
        GOODSNAME: info.NAME,
        GOODSID: info.ID,
        GOODSCODE: info.CODE,
  ...
  methods: {
    ...
    totalfun() {
      let tprice = 0, param = this.theGoodsList
      for (let i = 0; i < param.length; i++) {
