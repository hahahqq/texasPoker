      ...
      <div class='tableStyle'>
        <el-table :data="GoodsIdList"
          ...
          <el-table-column label='操作'  fixed="right">
            <template slot-scope="scope">
                <el-button size='mini' type="text" v-if='scope.row.isEdit == false' :disabled='scope.row.REMARK == undefined' @click.stop='handledblClickRow(scope.row)'>编辑</el-button>
           ...
           console.log(data)
            ...
            if(data.success){
                ...
                for(var i in param){
                    ...
                    let discount = param[i].GOODSPURPRICE == 0 ? 1 : parseFloat(price / param[i].GOODSPURPRICE).toFixed(2)
                    param[i].idx = i
                    param[i].isEdit = false
                    param[i].PRICE = price == undefined ? 0 : price
                    param[i].Discount = param[i].REMARK == '货号不存在' ? '' : discount
    ...
    methods:{
        ...
        console.log(arr)
            ...
            for (let i = 1; i < arr.length; i++) {
                ...
                newData.push({
                    GoodsCode: strCode,
                    Price: arr[i].__EMPTY_2,
                    Qty: arr[i].__EMPTY_1 == undefined ? 1 : arr[i].__EMPTY_1
                })
            }
            this.$store.dispatch('uploadStockExcel', { GoodsList: JSON.stringify(newData)} ).then(()=>{
                this.loading = true
