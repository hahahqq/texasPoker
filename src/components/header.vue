	...
	<div>
		<el-row>
			<el-col :span="16" class="member-header">
				<div
					class="center-title"
					style="border-right: 0; padding-left: 10px"
					v-if="$route.name == '首页'"
				>
					{{ shopName }}
				</div>
				...
				<div
					...
					v-if="
						$route.name != '首页' &&
						$route.meta.title == '营销中心' &&
						$route.name != '拓客工具'
					"
				>
					<a
						style="color: #137deb; cursor: pointer"
						@click="$router.push({ path: '/marketing/bespeakList' })"
					>
						拓客工具
					</a>
					<i style="color: #868686">/</i>
					{{ $route.name }}
				</div>

				<div class="center-cont" v-if="$route.name == 'memberItem'">
					<a
						style="color: #137deb; cursor: pointer"
						@click="$router.push({ path: '/member/MemberList' })"
					>
						会员列表
					</a>
					<i style="color: #868686">/</i>
				...
				<div class="center-cont" v-if="$route.name == 'verityBill'">
					<a
						style="color: #137deb; cursor: pointer"
						@click="$router.push({ path: '/game/match/index' })"
					>
						赛事中心
					</a>
					<i style="color: #868686">/</i>
				...
				<div class="center-cont" v-if="$route.name == 'acceptanceItem'">
					<a
						style="color: #137deb; cursor: pointer"
						@click="$router.push({ path: '/member/acceptance' })"
					>
						意见受理
					</a>
					<i style="color: #868686">/</i>
				...
				<div
					class="center-cont"
					v-if="
						$route.name == 'sngDetails' ||
						$route.name == 'mttDetails' ||
						$route.name == 'integralDetails'
					"
				>
					<a
						style="color: #137deb; cursor: pointer"
					...
					<i style="color: #868686">/</i>
					<span v-if="$route.name == 'sngDetails'">SNG</span>
					<span v-if="$route.name == 'mttDetails'">MTT</span>
					<span v-if="$route.name == 'integralDetails'">积分赛</span>
					<span v-if="$route.name == 'verityBill'">报名核销</span>
					赛事详情
				</div>

				<div
					class="center-cont"
					v-if="$route.name == '拓客工具' && $route.meta.title == '营销中心'"
				>
					{{ $route.name }}
				</div>
				...
				<div
					...
					v-else-if="
						...
						$route.name != 'mttDetails' &&
						$route.name != 'integralDetails' &&
						$route.name != 'memberItem' &&
						$route.name != 'acceptanceItem' &&
						$route.name != 'verityBill'
					"
				>
					{{ $route.name }}
				</div>
