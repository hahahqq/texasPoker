  ...
  <div class="chongzhi">
    ...
    >
      ...
      <el-row :gutter="10">
        ...
        <el-col :xs="24" :sm="12">
          <el-form-item label="当前储值积分">
            <el-input
              v-model="vipnews.MONEY"
              disabled
              placeholder="当前储值积分"
              size="small"
              style="width: 100%"
          ...
          <el-form-item>
            <ul v-if="storagevaluerroyaltyState.data.GiftList.length != 0">
              <li
                ...
                :style="`background: ${
                  curPayWayIdx == index ? '#ecf5ff' : '#ffffff'
                }; border: 1px solid ${curPayWayIdx == index ? '#b3d8ff' : '#ddd'} `"
              >
                <div class="addMoney" style="">充{{ item.ADDMONEY }} 元</div>
                <div class="giftMoney" style="">送 {{ item.MONEY }} 元</div>

                <div v-if="curPayWayIdx == index" :class="{ selected: curPayWayIdx == index }">
            ...
            <div class="rightList1">
              <div class="showpayList overflowscroll">
                <ul>
                  ...
                  >
                    <div class="paylistsock" :style="`background: ${
                      curtab == index ? '#ecf5ff' : '#ffffff'
                    }; border: 1px solid ${curtab == index ? '#b3d8ff' : '#ddd'} `" style="padding-top: 10px">
                      <img
                        :src="item.payerIMG"
                        alt=""
                        style="width: 28px; height: 30px; display: table; margin: 0 auto"
                      />
...
export default {
  ...
  watch: {
    ...
    rechargeListList1(data) {
      let newParam = data.filter((item) => item.NAME != "积分支付" && item.NAME != "欠款");
...
.rechargeWayItem .selected {
  ...
  bottom: 0px;
  right: 0px;
  border-bottom: 26px solid #137deb;
