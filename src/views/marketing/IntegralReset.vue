		...
		<el-container>
			...
			<el-container>
				<div class="content-new-fex">
					...
					<el-dialog
						:title="title"
						:visible.sync="showEditAddDialog"
						append-to-body
						width="30%"
					>
						...
						>
							<el-button
								type="primary"
								style="width: 100%"
								@click="addIntegralResetSubmit"
							>
								确 认
								<span v-if="isCheckSms">
									（ 剩余短信 {{ companyInfo.SMSNumber }} 条 ）
								</span>
							</el-button>
						</div>
					</el-dialog>
...
export default {
	...
	methods: {
		...
		handleView(row, idx) {
			...
			this.$store
				.dispatch("detailIntegralReset", { BillId: row.BILLID, PN: this.curVipPn })
				.then(() => {
					this.showEditAddDialog = true;
				});
		},
