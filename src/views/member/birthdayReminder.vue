                  ...
                  <el-table-column prop="NAME" label="会员信息" width="160">
                     <template slot-scope="scope">
                           ...
                           <span style='height:40px;width:102px'>
                              ...
                              <i class="pull-left inline-block" style="width:92px;overflow: hidden; text-overflow:ellipsis;white-space: nowrap ">{{scope.row.MOBILENO}}</i>
                           </span>
                     </template>
                  </el-table-column>
...
export default {
   ...
   computed: {
        ...mapGetters({
           vipBirthdayReminderListState: 'vipBirthdayReminderListState'
        })
   },
   watch: {
      vipBirthdayReminderListState(data){
         console.log(data)
         this.loading = false
         if(data.success){
   ...
   methods: {
      ...
      getNewDataFun(){
         this.$store.dispatch('getVipBirthdayReminder', this.ruleFrom).then(() => {
            this.loading = true
         })
      },
