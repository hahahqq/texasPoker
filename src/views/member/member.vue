<template>
    <el-container>
        <input type="file" ref="upload" @change="readExcel" accept=".xls, .xlsx" class="outputlist_upload hide" />
            ...
            <el-container>
                <el-main>
                    <div class="member-main-top">
                        <div class="content-center">
                            <div class="member-main-top-buttom">
                                <div class="btn">
                                    ...
                                    <div class="btm-list"><el-button plain size="small" @click="inportMerberData=true">会员导入</el-button></div>
                                    <div class="btm-list"><el-button plain size="small" @click="ExportMemberData" :loading="exportLoading">会员导出</el-button></div>
                                </div>
                                <div class="ipt">
                            ...
                            <div class="top-input">
                                <div class="top-input-conts">
                                    <div>
                                        ...
                                        <el-select v-model="value2" placeholder="请选择" size="small" style="width:200px" @change="selectShop" clearable>
                                            <el-option
                                    ...
                                    <div class="top-input-list">
                                        ...
                                        <el-select v-model="value3" placeholder="请选择" size="small" style="width:200px" @change="selectShop" clearable>
                                            <el-option
                                ...
                                <div>
                                    <el-form label-width="100px">
                                        <el-form-item label="会员标签:">
                                            <ul class="memberType">
                                                <li v-for="(item,index) in datamemberLeveList" :key="index" @click="secltListCont(index,item)" :class="item.isSelect==true?'memberType-secect':''">{{item.VIPFLAG}}</li>
                                            </ul>
                                        </el-form-item>
                                        <el-form-item label="销售顾问:">
                                            <ul class="memberType">
                                                <li v-for="(item,index) in disoptionsman" :key="index" @click="secltListContType(index,item)" :class="item.isSelect==true?'memberType-secect':''">{{item.label}}</li>
                                            </ul>
                                        </el-form-item>
                                        <el-form-item label="会员生日:">
                                            <ul class="memberType">
                                                <li v-for="(item,index) in birthdayDate" :key="index" @click="secltListBirthday(index,item)" :class="item.isSelect==true?'memberType-secect':''">{{item.name}}</li>
                                            </ul>
                                        </el-form-item>
                                        <el-form-item label="消费次数:">
                                            <ul class="memberType">
                                                <li v-for="(item,index) in PayCount" :key="index" @click="secltListConsume(index,item)" :class="item.isSelect==true?'memberType-secect':''">{{item.name}}</li>
                                            </ul>
                                        </el-form-item>
                                        <el-form-item label="流失客户:">
                                            <ul class="memberType">
                                                <li v-for="(item,index) in memberWastage" :key="index" @click="secltListWastage(index,item)" :class="item.isSelect==true?'memberType-secect':''">{{item.name}}</li>
                                            </ul>
                                        </el-form-item>
                                    </el-form>
                                </div>
                        ...
                        <!-- 新增会员 -->
                        <el-dialog v-if="showAddNew" title="新增会员" :visible.sync="showAddNew" width="850px" style="max-width:100%" >
                            <add-new-member @closeModal="showAddNew=false" @resetList="showAddNew=false;getNewData();"></add-new-member>
                        </el-dialog>
                        ...
                        <el-dialog title="会员导入" :visible.sync="inportMerberData" width="40%">
                            <span class="full-width pull-left">
                                ...
                                <span v-if="tableUploadData.length == 0">
                                    <el-button class="m-left-sm" size="small" type="primary" icon for="input"  @click="$refs.upload.click()"> 选择文件  </el-button>
                                    <span class="m-left-sm text-86">未选择任何文件</span>
                                </span>

                                <span v-else class="text-primary m-left-sm">共检索出 {{tableUploadData.length}} 条会员数据</span>
                            </span>

                            <div class="full-width m-top-md m-bottom-md pull-left" >
                                <a href="static/images/会员导入模板.xls" style="display:table; color:#3278b3; float:left" >模板下载</a>
                                <span class='text-86'>( 请按照模板中的要求整理会员信息 )</span>
                            </div>

                            <div class="text-right full-width">
                                <el-button size="small" type="primary" @click="okUpload">确定</el-button>
                                <el-button size="small" plain @click="inportMerberData = false; tableUploadData = []; importLoading = false">取消</el-button>
                            </div>
                        </el-dialog>
                    </div>
                        ...
                        <div class="content-table-center">
                            ...
                            >
                                ...
                                <el-table-column label="生日时间" align='center'>
                                   <template slot-scope="scope">
                                       <span v-if='scope.row.BIRTHDATE !=0 && scope.row.BIRTHDATE !=undefined'>{{new Date(scope.row.BIRTHDATE) | time}} </span>
                                       <span v-else> -- </span>
                                   </template>
                                </el-table-column>
  ...
  mixins: [MIXINS_MEMBER.MEMBER_MENU,  MIXNINS_EXPORT.TOEXCEL, MIXNINS_EXPORT.TODATA],
    data() {
        return {
            ...
            tableUploadDataCard:[],
            disoptionsman: [],
            birthdayDate:[{id:'-1',name:'全部',isSelect:true},{id:'1',name:'今天',isSelect:false},{id:'7',name:'明天',isSelect:false},{id:'30',name:'近30天',isSelect:false}],
    ...
    watch: {
         memberItemInfo(data){
            this.$router.push({
               name: 'memberItem', query:{ ID : this.vipID }
            })
         },
            ...
            if(this.inportMerberData==true){
                ...
                for (let i = 1; i < arr.length; i++) {
                    ...
                    let item = {
                        ...
                        MobileNo:
                            arr[i].__EMPTY_2 == undefined ? "" : Number(arr[i].__EMPTY_2),
