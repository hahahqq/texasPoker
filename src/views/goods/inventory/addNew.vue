<template>
  <div class="inventory">
      ...
      >
        ...
        <el-table-column label="单价" align="center" width="160">
          <template slot-scope="scope">
            ...
            <el-input-number v-if='scope.row.isEdit' size='small' style="width:140px" controls-position="right" v-model.trim="scope.row.PRICE" placeholder="请输入"></el-input-number>
          </template>
        </el-table-column>
        ...
        <el-table-column label="数量" align="center" width="160">
          <template slot-scope='scope'>
            ...
            <el-input-number v-if='scope.row.isEdit' size='small' style='width:140px' controls-position="right" v-model.trim="scope.row.QTY" placeholder="请输入"></el-input-number>
          </template>
        </el-table-column>
    ...
    <el-row :gutter="10" class="marginTB-sm">
      <el-col :xs="18" :sm="8" :md="10" class="row-flex flex-items-center">
    ...
    <div class='marginTB-sm'>
      ...
      <span class='text-right' style='float:right'>
        ...
        <el-button type="primary" @click="handleSubmit(1)" :disabled="theGoodsList[0].GOODSNAME == ''" :loading="loading">盘点</el-button>
      </span>
    </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    twoDisConfirm(){
      ...
      if(this.theGoodsList.length == 0){
        ...
        let arr2 = this.theGoodsList.concat(insertModels), newJson = []
        for(let item1 of arr2){
          let flag = true; 
          for(let item2 of newJson){
            if(item1.COLORID==item2.COLORID && item1.SIZEID==item2.SIZEID && item1.GOODSNAME == item2.GOODSNAME && item1.GOODSID == item2.GOODSID){ 
              flag = false;
            }
          }
          if(flag == true){ //判断是否重复
    ...
    handleSubmit(value) {
      ...
      this.$store.dispatch('addInventory',sendData).then(()=>{
        if(value == 0){
          this.CGloading = true
        }else{
          this.loading = true
  ...
  mounted() {
    ...
    let getItemData = sessionStorage.getItem('theGoodsList_I') || '[]'
    ...
    if(theGoodsList.length != 0){
        ...
        let getItemObj = sessionStorage.getItem('theGoodsObj_I')
