      ...
      <div class="content-table-center">
        <div class="tableStyle">
          ...
          >
            ...
            <el-table-column width="130" label="单价" align="center">
              <template slot-scope="scope">
          ...
          <el-col :xs="24" :sm="12" :md="12" class="row-flex flex-items-center">
            <span class="inline-block text-center">
              ...
              <i class="font-600 text-danger m-left-md font-18">
                {{ isPurViewFun(210040601) ? "&yen; " + totalPrice : "****" }}
              </i>
            </span>
          </el-col>
        </el-row>
...
export default {
  ...
  watch: {
    ...
    returnAddState(data) {
      ...
      if (data.success) {
        ...
        this.pageData.OTHERMONEY = 0;
        this.pageData.BREAKSMONEY = 0;
        this.pageData.PAYMONEY = 0;
        this.pageData.REMARK = "";
        this.$store.dispatch("getReturnItem", { BillId: data.data.OutBillId });
      } else {
  ...
  methods: {
    ...
    addNewBill() {
      // 新增单据
      let haveData = this.theGoodsList.filter(
    ...
    handleSubmit(value) {
      ...
      this.$store.dispatch("addReturn", sendData).then(() => {
        if (value == 0) {
          this.CGloading = true;
        } else {
          this.loading = true;
  ...
  mounted() {
    ...
    let getItemData = sessionStorage.getItem("theGoodsList_R") || "[]";
    ...
    if (theGoodsList.length != 0) {
      ...
      //   isEdit: true
      // });
      this.arrConcatFun(theGoodsList)

      let getItemObj = sessionStorage.getItem("theGoodsObj_R");
...
.return .el-input-group__prepend {
