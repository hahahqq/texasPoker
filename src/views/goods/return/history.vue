        ...
        <div class='history_top'>
          <el-row>
            ...
            <el-col :span="5" style='text-align:right'>
              <el-input size="small" v-model='searchText' placeholder="输入单号、供应商、店铺" @input='searchList(searchText)' clearable></el-input>
            </el-col>
            <el-col :span="3">
      ...
      <div class="content-table-center">
        <div class='history_content'>
          <el-table 
            ...
            <el-table-column label="供应商名称" prop='SUPPLIERNAME' width="110px" fixed="left"></el-table-column>
            ...
            <el-table-column label="退款金额" prop='PAYMONEY' align="right">
              <template slot-scope="scope">
                {{isPurViewFun(210040601) ? scope.row.PAYMONEY : '****'}}
              </template>
            </el-table-column>
            <el-table-column label="退货金额" align="right">
              <template slot-scope="scope">
                {{isPurViewFun(210040601) ? scope.row.GOODSMONEY : '****'}}
              </template>
            </el-table-column>
            ...
            <el-table-column label="操作" fixed='right' width="100">
              <template slot-scope="scope">
                <el-button size='small' type='text' @click='viewCurInfo(scope.row, scope.$index)'>详情</el-button>
                <el-button size='small' v-if='scope.row.ISCHECK == true && scope.row.ISCANCEL == false' type='text' @click='cancelBill(scope.row.BILLID,"zf")'>作废</el-button>
                <el-button size='small' v-if='scope.row.ISCHECK == false && scope.row.ISCANCEL == false' type='text' @click='cancelBill(scope.row.BILLID,"sc")'>删除</el-button>
              </template>
            </el-table-column>
          </el-table>
          <!-- 分页 -->
    ...
    <el-dialog :title='`详情 (${title})`' :visible.sync="isShowDialog" width="80%" append-to-body>
      ...
      <el-table
        ...
        <el-table-column prop="PRICE" label="单价" align="center" >
          <template slot-scope="scope">
            {{isPurViewFun(210040601) ? scope.row.PRICE : '****'}}
          </template>
        </el-table-column>
        <el-table-column prop="QTY" label="数量" align="center"></el-table-column>  
        <el-table-column label="金额" align="center" >
          <template slot-scope='scope'>
            {{isPurViewFun(210040601) ? (scope.row.PRICE * scope.row.QTY) : '****'}}
          </template>
        </el-table-column>
      </el-table>
      ...
      <el-row :gutter="24" class="marginTB-sm">
        ...
        <el-col :span="4">
          <span>实付金额：</span>
          <span>{{isPurViewFun(210040601) && pageData.PAYMONEY != undefined ? pageData.PAYMONEY : '****'}} </span>
        </el-col>
        <el-col :span="8">
          <span>总合计：</span>
          <span class="inline-block text-danger font-600" style="font-size:18px">{{isPurViewFun(210040601) ? '&yen;' + totalPrice : '****'}} 元</span>
        </el-col>
      </el-row>
  ...
  watch:{
    ...
    dataExportState(data){
      if(data.success){
        ...
        var head = ['状态','供应商名称','业务日期','单据编号','退货店铺','退货数量','退货金额','优惠金额','其它费用','退款金额','备注']
        var val =['STATUS','SUPPLIERNAME','BILLDATE','BILLNO','SHOPNAME','GOODSQTY','GOODSMONEY','BREAKSMONEY','OTHERMONEY','PAYMONEY','REMARK']
  ...
  methods:{
    getSummaries1(param) {
      ...
      columns.forEach((column, index) => {
        ...
        if(index == 7){
          if(!this.isPurViewFun(210040601)){
            sums[index] = '****'
            return
          }
        }
        if(index == 11){
          sums[index] = this.isPurViewFun(210040601) ? this.historyListSum : '****'
          return
        }
    ...
    getSummaries(param) {
      ...
      columns.forEach((column, index) => {
        ...
        if(index == 9){
          sums[index] = this.isPurViewFun(210040601) ? this.tableSum : '****'
          return
        }
        const values = data.map(item => Number(item[column.property]));
        if (!values.every(value => isNaN(value))) {
    ...
    handlePrint(isPrintStatu, BillId, item){  //弹窗列表是否打印状态
      if(isPrintStatu =='打印'){
        getDayindate('601021232', BillId, 15, '');
      } else{
    ...
    handleDraft(editStatu, BillId){  //弹窗列表是否作废状态
      if(editStatu == '作废'){  
        ...
        this.$store.dispatch('cgdReturnDataList', sendData).then(()=>{
          this.isShowDialog = false
