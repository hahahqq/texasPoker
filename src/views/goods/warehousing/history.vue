  ...
  <div class='history'>
        ...
        <div class="content-center">
          <el-row>
            ...
            <el-col :span="5" style='text-align:right'>
              <el-input size="small" v-model.trim='searchText' placeholder="输入单号、供应商、店铺" @keyup.enter.native='searchList(searchText)' @input='searchList(searchText)' clearable></el-input>
            </el-col>
            <el-col :span="3">
    ...
    <el-dialog :title='`详情 (${title})`' :visible.sync="isShowDialog" width="80%" append-to-body>
      ...
      <el-row :gutter="24" class="marginTB-sm">
        ...
        <el-col :span="4">
          <span>实付金额：</span>
          {{isPurViewFun(210040601) && pageData.PAYMONEY != undefined ? pageData.PAYMONEY : '****'}}
        </el-col>
        <el-col :span="8">
          <span>总合计：</span>
  ...
  watch:{
    dataExportState(data){
      if(data.success){
        ...
        var head = ['状态','供应商名称','业务日期','单据编号','入库店铺','采购数量','采购金额','优惠金额','其它费用','实付金额','备注']
  ...
  methods:{
    ...
    getSummaries(param) {
      ...
      columns.forEach((column, index) => {
        ...
        if(index == 9){
          sums[index] = this.isPurViewFun(210040601) ? this.dataObj.GOODSMONEY : '****'
          return
    ...
    cancelBill(value, status){
      ...
      if(status == 'zf' && !this.isPurViewFun(210040304)){
    ...
    handlePrint(isPrintStatu, BillId, item){  //打印
      if(isPrintStatu =='打印'){
        getDayindate('601021222', BillId, 14, '')
      } else{
    ...
    handleDraft(editStatu, BillId){
      if(editStatu == '作废'){
        let agentPermission = getUserInfo().List
        if(!this.isPurViewFun(210040304)){
          this.$message.warning('您还没有获得相关权限!')
        }else{
        ...
        this.$store.dispatch('cgdWarehousingDataList', sendData).then(()=>{
          this.isShowDialog = false
