        ...
        <el-row>
          ...
          <el-col :span="5" style="text-align: right">
            <el-input
              ...
              placeholder="输入单号、供应商、店铺"
              @keyup.enter.native="searchList(searchText)"
        ...
        >
          ...
          <el-table-column label="操作" fixed="right" width="100">
            <template slot-scope="scope">
              ...
              <el-button
                size="small"
                v-if="scope.row.ISCHECK == true && scope.row.ISCANCEL == false"
                type="text"
                @click="cancelBill(scope.row.BILLID, 'zf')"
              >
                作废
              </el-button>
              <el-button
      ...
      <el-dialog :title="`详情 (${title})`" :visible.sync="isShowDialog" width="80%" append-to-body>
        ...
        <el-row :gutter="24" class="marginTB-sm">
          <el-col :span="4">
            <span>优惠金额：</span>
            <span>
              {{ pageData.BREAKSMONEY != undefined ? pageData.BREAKSMONEY : "￥0.00 元" }}
            </span>
          </el-col>
          <el-col :span="4">
            <span>其它费用：</span>
          ...
          <el-col :span="8">
            <span>总合计：</span>
            <span class="inline-block text-danger font-600" style="font-size: 18px">
              {{ isPurViewFun(210040601) ? "&yen;" + totalPrice : "****" }} 元
            </span>
          </el-col>
        </el-row>
  ...
  watch: {
    ...
    dataState(data) {
      ...
      if (data.success) {
        ...
        this.totalPrice =
          this.dataObj.GOODSMONEY + this.dataObj.OTHERMONEY - this.dataObj.BREAKSMONEY;
      } else {
        this.$message.error(data.message);
      }
    },
