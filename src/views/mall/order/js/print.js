...
const printingFun = function (billData, goodsList, vipData, QRcodeUrl) {
    ...
    if (QRcodeUrl) {
        LODOP.ADD_PRINT_IMAGE(hPos, 0, QRcode_imgWH, QRcode_imgWH, "<img border='0' src='" + QRcodeUrl + "' />");
...
function dividingLine(width, title) {
    ...
    let fsize = fontSize, vh = hPos - (fontSize / 2);
    if (title) {
        let str = "<div style='text-align:center;font-size:"
            + fsize
