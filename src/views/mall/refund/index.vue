<template>
   <section class="mallOrder bg-white p-bottom-sm" v-loading="loading">
      ...
      <div class="content-table-center" ref="tabTable">
         <el-tabs v-model="activeName" @tab-click="handleClick">
            ...
            >
               ...
               <div class="List overflowY-scroll" ref="List" :style="{ height: ListHeight + 'px' }">
                        ...
                        <table class="full-width" style="border-spacing: 0px">
                              ...
                              <tr class="bg-f8 text-4e">
                                 <th :colspan="tableHead.length">
                                    <div style="user-select: text">
                                       ...
                                       <span class="m-right-sm">
                                          提交时间：{{ new Date(itemList.BILLDATE) | formatTime }}
                                       </span>
                                    </div>
                                 </th>
                              </tr>
                           ...
                           <tbody>
                              ...
                              >
                                 <td rowspan="1" width="20%">
                                    <div class="row-flex">
                                       ...
                                       <div class="full-width p-left-sm">
                                          <div class="text-overflow-fag2" style="min-height: 30px">
                                             {{ item.GOODSNAME }}
                                          </div>
                                          <div class="m-top-xs">
                                             <b>&yen;{{ item.PRICE }}</b>
                                             <span>&times;{{ item.QTY }}</span>
                                             <span class="pull-right" v-if="itemList.STATUS == 2">
                                                退款成功
                                             </span>
                                          </div>
                                       </div>
                                 ...
                                 <td v-if="i == 0" :rowspan="itemList.subList.length" width="20%">
                                    <div v-if="itemList.RECEIPTNAME" class="m-bottom-sm">
                                       <div>
                                    ...
                                    <div v-if="itemList.LOGISTICSCOMPANYNAME" class="m-bottom-sm">
                                       <span>配送方式</span>
                                 ...
                                 <td v-if="i == 0" :rowspan="itemList.subList.length" width="12%">
                  ...
                  <div v-if="dataList.length == 0" class="padding-md text-center text-muted">
                     <span>暂无单据</span>
                  </div>
               </div>
               <!-- 分页 -->
...
export default {
   ...
   methods: {
      ...
      handleButton(item) {
         ...
         if (s == 1) {
            this.$confirm("是否" + (this.resultState ? "同意" : "拒绝") + "退款?", "提示", {
            ...
            this.itemTitle = "单据详情（" + this.formatterState2(item) + "单据）";
      ...
      setListHeight() {
         if (this.$refs.tabTable) {
            let top = this.$refs.tabTable.getBoundingClientRect().top;
            let marginSpace = 10 * 3,
               pager = 42;
            this.$nextTick(() => {
               this.ListHeight = window.innerHeight - top - marginSpace - pager - 105;
            });
         }
      },
      defaultData() {}
