<template>
    <section class="mallhome">
        <el-container v-if="hDate != 0">
            <el-main :style="{height:height+'px'}" style="padding-top: 0 !important">
                <el-row :gutter="20" class="row1 bg-white no-m-right text-center font-14">
                    <el-col
                        v-for="(item,i) in row1"
                        :key="i"
                        :span="24 / row1.length"
                        class="paddingTB-lg"
                    >
                        <div class="m-bottom-sm font-26" style="color:#2589FF;">{{item.value}}</div>
                        <div class="font-18">
                ...
                <div class="bg-white m-bottom-sm font-14">
                    <div class="padding-sm text-left">
                        <div class="paddingTB-sm font-18">
                            <strong> {{ hDate > 90 ? '旗舰版' : '试用版' }} </strong>
                        </div>
                        ...
                        <div class="m-bottom-sm text-muted">
                            <div class>当前为{{ hDate > 90 ? '旗舰版' : '免费试用版' }}  </div>
                            <div class>{{ hDate > 90 ? '更多功能静候您的体验' : '升级到商城版享受更多服务' }}  </div>
                        </div>
                        <div class="m-top-md">
                            <el-button
                                ...
                                @click="levelAdd"
                            >
                                <strong class="text-white">立即升级或续费</strong>
                            </el-button>
                        </div>
                    </div>
                </div>
        ...
        <el-container v-else :style="`width: 100%; height:${height}px; background:#fff`">
            <div class="bg-white m-bottom-sm font-14" style="margin: 0 auto;">
                    <div class="padding-sm text-left" style="margin-top: 200px">
                        <div class="paddingTB-sm font-18">
                            <strong>已到期</strong>
                        </div>
                        <div class="m-bottom-sm">
...
export default {
    ...
    methods: {
        toFollowLink(path, number) {
            ...
            let arr = userInfo.List.filter(
                element => element.MODULECODE == number
            );
        ...
        dealQdate() {
            ...
            if (this.iDate) {
                let leftTime =
                    new Date(this.iDate).getTime() - new Date().getTime();
                if (leftTime > 0) {
        ...
        levelAdd() {
            this.$alert("电话：4006 337 277", "服务热线", {
