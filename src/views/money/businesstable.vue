                ...
                >
                  ...
                  <tbody v-for="(items, index) in moneyName" :key="index">
                    ...
                    <tr v-for="(item, index) in items.List" :key="index">
                      ...
                      <td
                        align="center"
                        style="padding: 0 4px; width: 200px"
                        v-if="index == 0"
                        :rowspan="items.List.length"
                      >
                        {{ item.SALEEMPNAME }}
                      </td>
                      <td
                      ...
                      <td
                        ...
                        :rowspan="items.List.length"
                      >
                        {{ items.ISCANCEL == false ? "已完成" : "已作废" }}
                      </td>
                      <td
                        align="center"
                        style="padding: 0 4px; width: 300px"
                        v-if="index == 0"
                        :rowspan="items.List.length"
                      >
                        {{ items.REMARK }}
                      ...
                      <td
                        align="center"
                        style="padding: 0 4px; width: 70px; color: #2589ff; cursor: pointer"
                        v-if="index == 0"
                        rowspan="40"
                        @click="detailedPage(items)"
                      >
                        详情
                      </td>
                    </tr>
...
export default {
  ...
  data() {
    return {
      ...
      title: "",
      Filter: "",
      dateBE: [new Date(this.getCustomDay(-30)).getTime(), new Date().getTime()],
      ruleFrom: {
        ShopId: "",
        BeginDate: "1",
