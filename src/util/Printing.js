...
var scaling = 0.375,
  ...
  fontSize = 12;
const pType = [
  { idx: 14, InterfaceCode: '601021242', name: '采购入库历史' },
];
...
const GoodsConsumption = function (dayinjson, ItemsHeaderType) {
  ...
  if (ItemsHeaderType == 1 || ItemsHeaderType == 3) {
    ...
    if (ItemsHeaderType == 1) {
      if (pageWidth == 800) {
        ...
        PrintLODOP_Text(hPos, gw1-40, pageWidth, rowHeight, "折扣", { Bold: 1 });
        PrintLODOP_Text(hPos, gw1-30 + gw2 * 1 - 5, gw2+10, rowHeight, "单价", { Alignment: 2, Bold: 1 });
        PrintLODOP_Text(hPos, gw1-20 + gw2 * 2 - 10, gw2, rowHeight, "数量", { Alignment: 2, Bold: 1 });
        PrintLODOP_Text(hPos, gw1-17 + gw2 * 3 - 10, pageWidth, rowHeight, "小计", { Bold: 1 });
        hPos += rowHeight;
        for (let i = 0; i < dayinjson.GoodsObj.length; i++) {
          ...
          // PrintLODOP_Text(hPos, 1, gw1 + 5, ROWHEIGHT, goodsName, { TextNeatRow: true, LineSpacing: 0.3, Bold: 1 });
           LODOP.ADD_PRINT_TEXT(hPos, 0, tw, ROWHEIGHT, goodsName,  { TextNeatRow: true, LineSpacing: 0.3, Bold: 1 });
           hPos += ROWHEIGHT;
           showPrintrules(1);
           let GOODSPRICE=dayinjson.GoodsObj[i].GOODSPRICE;

          PrintLODOP_Text(hPos, 0, pageWidth, rowHeight, GOODSPRICE, { Bold: 1 });
          PrintLODOP_Text(hPos, gw1-40, pageWidth, rowHeight, DISCOUNT, { Bold: 1 });
          PrintLODOP_Text(hPos, gw1-30 + gw2 * 1 - 5, gw2, rowHeight, PRICE, { Alignment: 2, Bold: 1 });
          PrintLODOP_Text(hPos, gw1-20 + gw2 * 2 - 15, gw2 + 10, rowHeight,  qty, { Alignment: 2, Bold: 1 });
          PrintLODOP_Text(hPos, gw1-20 + gw2 * 3 - 10, pageWidth, rowHeight, money, { Bold: 1 });
          hPos += ROWHEIGHT;
        }
      }else{
        PrintLODOP_Text(hPos, 1, pageWidth, rowHeight, "商品", { Bold: 1 });
        PrintLODOP_Text(hPos, gw1-15, pageWidth, rowHeight, "折扣", { Bold: 1 });
        PrintLODOP_Text(hPos, gw1 + gw2 * 1 - 15, gw2, rowHeight, "单价", { Alignment: 2, Bold: 1 });
        PrintLODOP_Text(hPos, gw1 + gw2 * 2 - 10, gw2, rowHeight, "数量", { Alignment: 2, Bold: 1 });
        ...
        for (let i = 0; i < dayinjson.GoodsObj.length; i++) {
          ...
          PrintLODOP_Text(hPos, gw1 + gw2 * 1 - 15, gw2 + 10, rowHeight, PRICE, { Alignment: 2, Bold: 1 });
          PrintLODOP_Text(hPos, gw1 + gw2 * 2 - 10, gw2, rowHeight,  qty, { Alignment: 2, Bold: 1 });
    ...
    showPrintrules(1);
    LODOP.ADD_PRINT_TEXT(hPos, gw1 + gw2 * 2, gw2, rowHeight, "剩余", { Alignment: 2, Bold: 1 });
    showPrintrules(1);
    hPos += rowHeight;
    for (let i = 0; i < dayinjson.GoodsObj.length; i++) {
      let goodsName = dayinjson.GoodsObj[i].GOODSNAME,
    ...
    for (let i = 0; i < dest.length; i++) {
      for (let j = 0; j < dest[i].data.length; j++) {
        ...
        PrintLODOP_Text(hPos, 1, gw1, ROWHEIGHT, goodsName, { TextNeatRow: true, LineSpacing: 0.3, Bold: 1 });
    ...
    PrintLODOP_Text(hPos, 140, pageWidth, rowHeight, showPurPrice == true ? PAYMONEY : '****', { Bold: 1 });
    hPos += rowHeight;
  }
  ...
  if (String(REMARK).length != 0) { // 备注
    ...
    // PrintLODOP_Text(hPos, 35, 146, rowHeight, REMARK, { Bold: 1 });
  ...
  if (ItemsHeaderType != 14 && ItemsHeaderType != 15 && ItemsHeaderType != 16) {
    ...
    if (!pringInfo || pringInfo.ddList[1].isShow) { // 注脚
      ...
      PrintLODOP_Text(hPos, 0, printW, 130, pringInfo.ddList[1].value, {
        Alignment: 2, TextNeatRow: true, LineSpacing: 0.5, Bold: 1
      })
      hPos += rowHeight;
    }
