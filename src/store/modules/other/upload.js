...
const actions = {
  ...
  saveImage({ commit ,state}, data) { // 图片字段保存
    let userInfo = getUserInfo();
    let num = (state.imagesListState.data&&state.imagesListState.data.ImageMaxNum)?parseInt(state.imagesListState.data.ImageMaxNum):0;
    let sendData = {
  ...
  getImagesList({ commit },type) {  // 图片列表
    let userInfo = getUserInfo();
    let sendData = {
