...
const actions = {
  getintegralAdjustList ({commit},data) { //积分调整统计
    ...
    let pn = parseInt(state.integralAdjustListState.paying.PN) +1;
    let sendData = {
      'InterfaceCode': '6010310',
  ...
  getbalanceAdjustList ({commit,state},data) { // 余额调整统计
    let userInfo= getUserInfo();
    let pn = parseInt(state.balanceAdjustListState.paying.PN) +1;
    let sendData = {
      'InterfaceCode': '6010309',
      'CompanyId': userInfo.CompanyID,
      ShopId:data.ShopId,
      BeginDate:data.BeginDate?data.BeginDate :1,
      EndDate:data.EndDate?data.EndDate : 9999999999999,
