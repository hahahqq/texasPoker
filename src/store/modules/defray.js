...
// actions
const actions = {
  getDefrayList({commit,state}, data) { // 费用支出列表
    let homeInfo = getHomeData();
    let pn = parseInt(state.defrayListState.paying.PN) +1;
    if(  state.defrayListState.paying.PageSize >= state.defrayListState.paying.TotalNumber){ pn=1;}
    let sendData=Object.assign({},data,{
        'InterfaceCode':'210020812',
  ...
  cancelDefray({commit}, data) { // 费用支出作废
    let homeInfo = getHomeData();
    let sendData ={
      'InterfaceCode':'210020811_1',
