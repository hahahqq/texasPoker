...
const actions = {
  ...
  clearCategoryAll({state}){
    ...
    state.selcategory={}
    state.categoryItem={}
    state.goodsListState2.paying.PN = 0;
    state.goodsList2 = []
  },
  ...
  [DEAL_CATEGORY_ITEM] (state, { data }) {
    if(data.success){
      if(selected.type=="edit"){
        let index = state.categoryList.findIndex(obj => Object.is(parseInt(selected.data.ID), parseInt(obj.ID)));
